...
class Admin extends Component {
    constructor() {
        ...
        this.extTempIncrease = this.extTempIncrease.bind(this);
        this.extTempDecrease = this.extTempDecrease.bind(this);
    ...
    componentDidMount() {
        ...
        this.socket.emit("init", {"page": "admin"});
    ...
    refresh() {
        this.socket.emit("refresh", {"page": "admin"});
    }
    ...
    click_comp(area) {
        ...
        if(this.state.response["components"][comp_id]) {
            this.socket.emit("update_comp", {"comp_id": comp_id, "engaged": false, "page": "admin"})
        }
        else {
            this.socket.emit("update_comp", {"comp_id": comp_id, "engaged": true, "page": "admin"})
        }
    }

    setTempIncrease() {
        this.socket.emit("change_set_temp", {"direction": "up", "page": "admin"})
    }

    setTempDecrease() {
        this.socket.emit("change_set_temp", {"direction": "down", "page": "admin"})
    }

    extTempIncrease() {
        this.socket.emit("change_ext_temp", {"direction": "up", "page": "admin"})
    }

    extTempDecrease() {
        this.socket.emit("change_ext_temp", {"direction": "down", "page": "admin"})
    }

    resetTemp() {
      this.socket.emit("reset_temp", {"page": "admin"})
    }

    setWeather(event) {
      this.socket.emit("set_weather", {"page": "admin", "weather": event.target.value})
    }

    setSpeed(event) {
      this.socket.emit("set_speed", {"page": "admin", "speed": event.target.value})
    }
    ...
    render() {
      return (
        <div className="admin-container">
            ...
            <ControlPanel
              water_to_heat={this.state.response["water_to_heat"]}
              temp_delta={this.state.response["temp_delta"]}
              wattage={this.state.response["wattage"]}
              extTempIncrease={this.extTempIncrease}
              extTempDecrease={this.extTempDecrease}
